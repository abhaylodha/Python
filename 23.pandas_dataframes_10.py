...
df = pd.read_csv(os.path.dirname(os.path.abspath(__file__)) + '\\olympics.csv',
...
def answer_one():
    max_val = df["Gold"].max()
    return df[df["Gold"] == max_val].index[0]
...
def answer_two():
    df['Gold_Difference'] = (df['Gold'] - df['Gold.1']).abs()
    max_val = df["Gold_Difference"].max()
    return df[df["Gold_Difference"] == max_val].index[0]
...
def answer_three():
    ...
    df1['ratio'] = (df['Gold'] - df['Gold.1']).abs()/(df['Total'] + df['Total.1'])
    max_val = df1["ratio"].max()
    return df1[df1["ratio"] == max_val].index[0]
...
# [United States Census Bureau](http://www.census.gov). Counties are political 
# and geographic subdivisions of states in the United States. This dataset contains
...
#  /file-layouts/2010-2015/co-est2015-alldata.pdf) 
...
census_df = pd.read_csv(os.path.dirname(os.path.abspath(__file__)) + '\\census.csv')
...
def answer_five():
    ...
    print("SUMLEV = 50")
    print(df)
    groupedDF = df.groupby('STNAME').count()
    print("STNAME wise count")
    print(groupedDF)
    max_val = groupedDF['CTYNAME'].max()
    print("STNAME wise count's max value")
    print(max_val)
    return groupedDF[groupedDF["CTYNAME"] == max_val].index[0]
...
def answer_six():
    df = census_df[census_df['SUMLEV'] == 40].sort_values(by=['CENSUS2010POP'], ascending = False)
    return df.head(3)['CTYNAME'].values.tolist()

print(answer_six())

### Question 7
# Which county has had the largest absolute change in population 
# within the period 2010-2015? (Hint: population values are stored 
# in columns POPESTIMATE2010 through POPESTIMATE2015, you need to 
# consider all six columns.)
#
# e.g. If County Population in the 5 year period is 100, 120, 
# 80, 105, 100, 130, then its largest change in the period 
# would be |130-80| = 50.
#
# *This function should return a single string value.*

def answer_seven():
    df = census_df[census_df['SUMLEV'] == 40][['CTYNAME','POPESTIMATE2010', 'POPESTIMATE2011', 
               'POPESTIMATE2012', 'POPESTIMATE2013', 'POPESTIMATE2014', 'POPESTIMATE2015']]
    #df.reset_index()
    #df.set_index(['CTYNAME'])
    df['min']=df[['POPESTIMATE2010', 'POPESTIMATE2011', 
               'POPESTIMATE2012', 'POPESTIMATE2013', 
               'POPESTIMATE2014', 'POPESTIMATE2015']].min(axis=1, numeric_only =True)
    df['max']=df[['POPESTIMATE2010', 'POPESTIMATE2011', 
               'POPESTIMATE2012', 'POPESTIMATE2013', 
               'POPESTIMATE2014', 'POPESTIMATE2015']].max(axis=1, numeric_only =True)

    df['diff'] = df['max'] - df['min']
    max_val = df['diff'].max()
    return df[df['diff'] == max_val]['CTYNAME'].values.tolist()[0]
